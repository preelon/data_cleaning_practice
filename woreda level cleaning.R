...
not_matched_hmis_ahri <- hmis_ahri_joined %>%
  ...
  distinct(region, zone, woreda) #resulted 2 observations

#Lets deal with the 2 unmatched woredas
...
#lets check if the 2 unmatched woredas are resolved
#rejoin 1st 
hmis_ahri_joined <- mal_2017_2024 %>%
  left_join(ahri_matched, by= c("region" = "dhis2_region",
                                "zone" = "dhis2_zone",
...
not_matched <- hmis_ahri_sf_joined %>%
  ...
  distinct(region, zone, zone_1082, woreda, woreda_1082) #183 woredas are unmatched, initially was 295
...
not_matched <- hmis_ahri_sf_joined %>%
  filter(is.na(id_1082)) %>%
  distinct(region, zone_1082, woreda_1082) # now there are 12 unmatched woredas

#lets deal with the remaining 12 unmatched woredas
hmis_ahri_joined <- hmis_ahri_joined %>%
